  ...
  <q-layout view="lHh Lpr lFf">
    <q-header class="bg-white" elevated>
      <q-toolbar class="q-py-md">
        <q-btn aria-label="Menu" color="primary" dense flat icon="menu" round @click="toggleLeftDrawer" v-if="isSeller" />
        <div class="text-white fredoka text-h6">
          <router-link class="text-primary q-mx-lg" style="text-decoration: none" to="/">App
          </router-link>
        </div>
        <q-space />
      </q-toolbar>
    </q-header>
    <q-drawer v-model="leftDrawerOpen" behavior="mobile" bordered class="bg-dark text-white" v-if="isSeller" :width="200">
      <q-list class="full-height">
        <q-item :clickable="false" :link="false" class="bg-primary q-mb-md">
          <q-item-section>
        ...
        <div v-if="!isAuthenticated" class="q-mx-md q-gutter-md">
          <q-btn class="full-width" color="primary" label="Login" to="/login" />
          <q-btn class="full-width" color="transparent" label="Cadastro" to="/shop/register" />
        </div>
      </q-list>
    </q-drawer>
    <q-page-container>
